...
def render():
    for i, j in pixels:
        ...
        newu = 0.85*lap(u, i, j) - x*y*y + 0.986*x + 0.036
        newv = 0.15*lap(v, i, j) + x*y*y + 0.93*y -0.01

        du = (ti.cast(i, ti.f32)/res[0])
        dv = (ti.cast(j, ti.f32)/res[1])

        # newu = 0.85*du*lap(u, i, j) - x*y*y + 0.986*x + 0.02
        ...
        pixels[i, j] = hsv_to_rgb(2.1+u[i,j], 0.1+ 0.05*u[i, j], 0.02+0.78*u[i, j])
